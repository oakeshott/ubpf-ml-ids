...
def compute_flow_features(group: pd.DataFrame, obs_time: Optional[float]):
    ...
    fwd_pkt_len_var  = float(np.var(lengths)) if n_packets > 1 else 0.0
    ...
    if n_packets > 1:
        iats_ms = np.diff(times_ns).astype(float) / 1e3
        ...
        fwd_iat_var = float(np.var(iats_ms))
    else:
        fwd_iat_total = 0.0
        fwd_iat_mean = 0.0
        fwd_iat_max = 0.0
    ...
    return {
        ...
        "Total Fwd Packets": n_packets,
        "Total Length of Fwd Packets": total_len_fwd,

        "Fwd Packet Length Max": fwd_pkt_len_max,
        "Fwd Packet Length Min": fwd_pkt_len_min,
        "Fwd Packet Length Mean": fwd_pkt_len_mean,
        "Fwd Packet Length Var": fwd_pkt_len_var,

        "Fwd IAT Total": fwd_iat_total,
        "Fwd IAT Mean": fwd_iat_mean,
        "Fwd IAT Max": fwd_iat_max,
        "Fwd IAT Min": fwd_iat_min,
        "Fwd IAT Var": fwd_iat_var,
